<?php
$user=  \common\models\UserAddress::find()->where(['user_id'=> $merchant->user_id])->one();
$order_data =  \common\models\Orders::findOne($merchant->order_id);
$user_address=  \common\models\UserAddress::find()->where(['id' => $order_data->ship_address_id])->one();
$billing_address =  \common\models\UserAddress::find()->where(['id' => $order_data->bill_address_id])->one();
//$order_details=  \common\models\OrderedProducts::find()->where(['order_id'=>$order->id,'merchant_id'=>$merchant_id])->all();
...
<html>
        <head>
                <title>Dunia.com: Order Return # <?php echo $order->id; ?></title>
        ...
        <body bgcolor="#FFFFFF" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0">
                ...
                <div style="margin:auto; width:776px; border:solid 2px #404241; margin-top:40px; margin-bottom:40px;">
                        <table id="Table_01" width="776" border="0" cellpadding="0" cellspacing="0" align="center" style=" font-family: 'Open Sans',arial, sans-serif;">
                                ...
                                <tr>
                                        <td valign="top">
                                                <h1 style="font-size:22px;font-weight:normal;line-height:22px;margin:13px 0 12px 9px;text-align:left;">Hello, Seller
                                                ...
                                                <p style="font-size:13px;line-height:16px;margin: 0px 12px 8px 9px;text-align:left;">
                                                        A order from   <?php echo $user->first_name." ".$user->last_name; ?><small>(placed on <?php
                                                                echo
                                                                date("F d , Y", strtotime($order_data->created_on));
                                                                ?>)</small>
                                                </p>
                                        </td>
                                </tr>
                                                                ...
                                                                <tr>
                                                                        ...
                                                                        <td valign="top" style="font-family: 'Open Sans',arial, sans-serif;font-size:12px;padding:7px 9px 9px 9px;border-left:1px solid #eaeaea;border-bottom:1px solid #eaeaea;border-right:1px solid #eaeaea">
                                                                                <p style="text-transform: uppercase;font-weight: bold;padding-top:20px;">pay via <?php
                                                                                        ...
                                                                                        } elseif ($order_data->payment_mode == 2) {
                                                                                                echo "Paypal";
                                                                                        } elseif ($order_data->payment_mode == 3) {
                                                                                                echo "Cash On Delivery";
                                                                                        } elseif ($order_data->payment_mode == 4) {
                                                                                                $wallet_amt = $order->wallet;
                                                                                                if ($order_data->netbanking != '') {
                                                                                                        $payment_amt = $order->netbanking;
                                                                                                        $method = 'CREDIT/DEBIT CARD OR NET BANKING';
                                                                                                } else if ($order_data->paypal != '') {
                                                        ...
                                                        <tbody bgcolor="#F6F6F6">
                                                         ...
                                                         $shipping=0;   
                                                             //   foreach ($order_details as $orders)  {
                                                                 $product_names = \common\models\Products::find()->where(['id' => $merchant->product_id])->one();
                                                                  $product=  common\models\ProductDescription::find()->where(['product_id'=>$product_names->id])->one();     
                                                                $sub = $merchant->amount * $merchant->quantity; 
                                                                    $get_options = explode(',', $merchant->options);
                                                                        ?>
                                                                        <tr>
                                                                                ...
                                                                                <td align="center" valign="top" style="font-size:11px;padding:3px 9px;padding-top:10px;padding-bottom:10px;border-bottom:1px dotted #cccccc;"><?php echo $merchant->quantity; ?></td>
                                                              ...
                                                              <?php
                                                              $total = $sub;
                                                              $shipping = $merchant->shipping_charge;
                                                             //   }
